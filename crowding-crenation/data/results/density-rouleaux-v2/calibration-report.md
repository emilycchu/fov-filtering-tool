...
| edge_density_unmasked | 0.098 | [0.05926, 0.1811] |
| tile_glcm_cv | 0.082 | [0.06178, 0.3112] |
| tile_glcm_patchiness | 0.171 | [0.1247, 0.9341] |

**Cross-validation** (5-fold): per-fold rho = [0.789, 0.793, 0.789, 0.76, 0.782], mean=0.783. Out-of-fold exact-match=69.4%, off-by-one=98.0%.
...
## Rouleaux composite (v2.2-optimized)
...
| glcm_contrast | 0.098 | [26.59, 87.11] |
| edge_density_unmasked | 0.043 | [0.05926, 0.1811] |
| tile_glcm_cv | 0.089 | [0.06178, 0.3112] |
| tile_glcm_patchiness | 0.238 | [0.1247, 0.9341] |

**Cross-validation** (5-fold): per-fold rho = [0.711, 0.72, 0.745, 0.772, 0.738], mean=0.737. Out-of-fold exact-match=67.6%, off-by-one=93.8%.

Out-of-fold confusion matrix (rows=manual, cols=predicted):

| manual \ predicted | No Rouleaux | Slight Rouleaux | Some Rouleaux | Rouleaux | Heavy Rouleaux |
|---|---|---|---|---|---|
| No Rouleaux | 341 | 59 | 14 | 6 | 0 |
| Slight Rouleaux | 31 | 30 | 24 | 10 | 2 |
| Some Rouleaux | 2 | 16 | 20 | 5 | 3 |
| Rouleaux | 0 | 2 | 7 | 12 | 13 |
| Heavy Rouleaux | 0 | 0 | 2 | 18 | 44 |
...
| feature | weight (v2.2) | weight (optimized) | delta |
|---|---|---|---|
| coverage | 0.2356 | 0.2358 | +0.00022 |
| otsu_separability | 0.0140 | 0.0145 | +0.00051 |
| saturation_score | 0.2465 | 0.2466 | +0.00007 |
| lbp_entropy | 0.0654 | 0.0641 | -0.00129 |
| glcm_contrast | 0.0874 | 0.0876 | +0.00022 |
| edge_density_unmasked | 0.0976 | 0.0977 | +0.00016 |
| tile_glcm_cv | 0.0822 | 0.0823 | +0.00017 |
| tile_glcm_patchiness | 0.1714 | 0.1713 | -0.00005 |

Thresholds: [0.2916, 0.4423, 0.5845, 0.7096] vs. v2.2 [0.2917, 0.4427, 0.5853, 0.7109] (max shift 0.00131).

**Rouleaux**

| feature | weight (v2.2) | weight (optimized) | delta |
|---|---|---|---|
| coverage | 0.2633 | 0.2633 | +0.00001 |
| saturation_score | 0.2688 | 0.2688 | +0.00001 |
| glcm_contrast | 0.0976 | 0.0975 | -0.00007 |
| edge_density_unmasked | 0.0434 | 0.0434 | -0.00005 |
| tile_glcm_cv | 0.0884 | 0.0885 | +0.00014 |
| tile_glcm_patchiness | 0.2385 | 0.2385 | -0.00006 |

Thresholds: [0.3739, 0.4508, 0.5534, 0.6672] vs. v2.2 [0.3738, 0.4508, 0.5533, 0.6672] (max shift 0.00007).

## Composite independence (v2.2-optimized)

Spearman rho between the two fitted composite scores: **0.972**, vs. the true manual density-vs-Rouleaux label correlation of **0.823**.

## Axis-separation check (v2.2-optimized)
