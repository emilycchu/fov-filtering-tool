...
2. **Tanzania fallback added.** `tanzania_02032026`'s own `detection_results/` tree turned out to
   ...
   `fov_summary.csv` for all 146, and `crop_counts.py` now falls back to it. **The two copies are
   *not* byte-identical -- the "confirmed byte-identical where both exist" claim made here on
   2026-08-11 was wrong and was retracted 2026-08-12.** What is true: `n_spots_detected` and
   `n_spots_filtered` are identical per-FOV on all 8 slides that have both copies, but
   `n_positives` differs on 8/8 because the annotation bucket's copy comes from model O2.0 rather
   than `v8_hardneg_single_t0.995`. **This analysis's headline metric is `n_spots_detected`, so it
   is unaffected; the parasite-count variant is, and its caveat is in Caveats below.** See
   `crop_counts.py`'s module docstring for the full evidence, and
   `crop-outlier-v2/case-study-KTR-72502946.md` for a full-slide case study on the more
   interesting of the two recovered samples.
...
- **Tanzania fallback (2026-08-11).** `tanzania_02032026`'s own `detection_results/` tree is
  ...
  ~~Verified byte-identical to `tanzania_02032026`'s own data on a sample that has both.~~
  **Retracted 2026-08-12 -- see the next caveat.** Not
...
- **The Tanzania fallback mixes model versions, but only in the parasite-count variant (found
  2026-08-12).** The annotation bucket's `fov_summary.csv` is generated by model **O2.0**, not the
  `v8_hardneg_single_t0.995` this analysis standardizes Tanzania on. Verified on all 8 TZ slides
  here that have both copies: `n_spots_detected` and `n_spots_filtered` match identically per-FOV
  (spot-finding and filtering are upstream of the classifier and shared across model versions),
  while `n_positives` differs on 8/8 in both directions -- e.g. `KIT-62501035` 144 vs 46,
  `NKR-72502319` 23 vs 101. Identified via `model_predictions/O2.0/<sample>_O2.0.parquet`, whose
  `positive` count reproduces the annotation copy's `n_positives` exactly.
    * **`results.csv` and everything derived from it are unaffected** -- `n_spots_detected` is
      byte-for-byte the same either way. That covers the main tables, `boundary_negatives.csv`, the
      `robust_zscore >= 6` threshold, and the `KTR-72502946` case study.
    * **`results_parasites.csv` mixes versions in principle, but the numeric impact is currently
      nil.** Only 2 of its 12 TZ samples resolve via the fallback (`KIT-62500763`,
      `KTR-72502946`), accounting for 3 of 76 rows -- and all 3 are already flagged
      `zero_or_undefined_baseline` (`baseline_mad=0`, `baseline_median=0`) with a blank
      `robust_zscore`, so they contribute to no statistic reported anywhere. **No number in this
      README changes.** The hazard is forward-looking: extend the parasite variant to more slides,
      or to any of the other 143 gap slides, and O2.0 `n_positives` will silently mix with
      t0.995 `n_positives` in rows that *do* produce scores.
    * Fixing it properly means either regenerating `fov_summary.csv` for the gap slides at
      t0.995, or restricting the fallback to `n_spots_detected` and returning no data for
